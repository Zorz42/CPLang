...
fn check_refs_block(mut block: IRBlock, autorefs: &[i32]) -> CompilerResult<IRBlock> {
    let out: CompilerResult<Vec<_>> = block.statements.into_iter().map(|statement| check_refs_statement(statement, autorefs)).collect();
...
fn check_refs_statement(statement: IRStatement, autorefs: &[i32]) -> CompilerResult<IRStatement> {
    let res = match statement {
        ...
        IRStatement::Return { return_value } =>
            IRStatement::Return {
                return_value: return_value.map(|return_value| check_refs_expression(return_value, autorefs)).transpose()?.map(|x| x.0),
            },
...
fn check_refs_expression(expression: IRExpression, autorefs: &[i32]) -> CompilerResult<(IRExpression, ValuePhysicality)> {
    let res = match expression {
        ...
        IRExpression::InstanceCall { instance_label, instance_arguments } =>
            (
                IRExpression::InstanceCall {
                    instance_label,
                    instance_arguments: instance_arguments.into_iter().map(|arg| check_refs_expression(arg, autorefs))
                        .collect::<CompilerResult<Vec<_>>>()?.into_iter().map(|x| x.0).collect::<Vec<_>>(),
                },
        ...
        IRExpression::StructInitialization { struct_label, field_values, fields_type_labels } =>
            (
                IRExpression::StructInitialization {
                    struct_label,
                    fields_type_labels,
                    field_values: field_values.into_iter().map(|val| check_refs_expression(val, autorefs))
                        .collect::<CompilerResult<Vec<_>>>()?.into_iter().map(|x| x.0).collect::<Vec<_>>(),
                },
                ValuePhysicality::Temporary,
            ),
