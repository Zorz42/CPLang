...
pub fn lower_ast(mut ast: Ast) -> Ast {
    ...
    ast.functions = ast.functions.into_iter().map(|(mut sign, block)| {
        sign.args = sign.args.into_iter().map(|(arg_name, arg_type, arg_pos)| (arg_name, lowerer.lower_type(arg_type), arg_pos)).collect();
    ...
    fn lower_statement(&mut self, statement: ASTStatement) -> ASTStatement {
        match statement {
            ...
            ASTStatement::Print { values } => {
                let values = self.lower_expressions(values);
                let mut block = Vec::new();
