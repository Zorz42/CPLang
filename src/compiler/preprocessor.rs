...
impl Fragment {
    pub fn get_position(&self) -> FilePosition {
        match self {
            ...
            Fragment::BraceBlock(b) | Fragment::BracketBlock(b) | Fragment::ParenthesisBlock(b) => {
                b.position.clone()
            }
        }
    }
...
pub fn parse_strings_and_comments(input: &Vec<PosChar>) -> CompilerResult<Vec<Fragment>> {
    ...
    while let Some(pos_char) = chars.next() {
        match location.clone() {
            ...
            Location::String => {
                if pos_char.c == '"' {
                    let pos = merge_file_positions(
                        string_quote_position.as_ref().unwrap(),
                        &pos_char.pos,
                    );
...
fn parse_indentation_block(
    lines: &Vec<(i32, Vec<Fragment>)>,
    curr_idx: &mut usize,
) -> CompilerResult<FragmentBlock> {
    let mut res = Vec::new();
...
fn parse_indentation(input: &FragmentBlock) -> CompilerResult<FragmentBlock> {
    ...
    for (indent, line) in &mut lines {
        ...
        if leading_spaces % 4 != 0 {
            ...
            return Err(CompilerError {
                message: format!(
                    "Indentation must have a multiple of 4 spaces, found {} spaces",
                    leading_spaces
                ),
