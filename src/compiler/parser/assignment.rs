...
pub fn parse_assignment(
    ...
    let (symbol, symbol_pos) =
        if let Some((Token::Symbol(symbol), pos)) = block.children.get(*curr_idx).cloned() {
            match symbol {
                Symbol::Assign
                | Symbol::Increase
                | Symbol::Decrease
                | Symbol::Increment
                | Symbol::Decrement => (symbol, pos),
                _ => {
                    *curr_idx = old_idx;
                    return Ok(None);
                }
    ...
    *curr_idx += 1;
    ...
    let res = match symbol.clone() {
        Symbol::Assign | Symbol::Increase | Symbol::Decrease => {
            ...
            match symbol {
                Symbol::Assign => ASTStatement::Assignment {
                    assign_to: expr1,
                    value: expr2,
                    pos,
                },
        ...
        Symbol::Increment => ASTStatement::AssignmentIncrement {
            assign_to: expr1,
            pos: merge_file_positions(&expr1_pos, &symbol_pos),
        },
